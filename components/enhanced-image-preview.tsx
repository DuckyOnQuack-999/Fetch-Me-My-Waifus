...
interface EnhancedImagePreviewProps {
  ...
  onUpscale?: (image: WaifuImage) => void
...
export function EnhancedImagePreview({
  ...
  const favorited = isFavorite ? isFavorite(image.image_id) : false
      ...
      <div className="space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <Button
              variant="outline"
              className={`w-full justify-start bg-transparent ${favorited ? "text-red-500 border-red-500" : ""}`}
              onClick={() => {
                if (toggleFavorite) {
                  toggleFavorite(image.image_id)
                  toast.success(favorited ? "Removed from favorites" : "Added to favorites")
                } else {
                  toast.error("Favorites not available")
                }
              }}
            >
              <Heart className={`h-4 w-4 mr-2 ${favorited ? "fill-red-500" : ""}`} />
            ...
            <Button
              ...
              onClick={() => {
                toast.info("Select a collection from the Collections page")
              }}
            >
            ...
            <Button
              ...
              onClick={() => {
                if (onUpscale) {
                  ...
                  toast.error("Upscaler not available")
                }
              }}
            >
